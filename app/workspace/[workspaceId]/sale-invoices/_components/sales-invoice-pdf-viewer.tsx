...
import {
  ...
  type SalesInvoicePdfCompany,
  type SalesInvoicePdfData,
} from "./sales-invoice-pdf-document";
...
export default function SalesInvoicePdfViewer({ company, invoice }: Props) {
    ...
    (async () => {
      try {
        const blob = await pdf(
          <SalesInvoicePdfDocument company={company} invoice={invoice} />,
        ).toBlob();
        if (cancelled) return;
        ...
        console.error("[SalesInvoicePdfViewer] render failed", err);
        if (cancelled) return;
        setError(
          err instanceof Error
            ? err.message
            : "Couldn't render the invoice PDF.",
        );
      }
    })();
